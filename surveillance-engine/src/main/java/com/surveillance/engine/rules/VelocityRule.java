...
public class VelocityRule {
    ...
    public void then(@Fact("transaction") Transaction transaction) {
        ...
        List<Transaction> last5Min = repo.findByFromAccountAndCreatedAtAfter(
            from, now.minusMinutes(5));
        List<Transaction> lastHour = repo.findByFromAccountAndCreatedAtAfter(
            from, now.minusHours(1));
        List<Transaction> lastDay = repo.findByFromAccountAndCreatedAtAfter(
        ...
        if (failed >= 2) {
            ...
            transaction.setManagerHint("🚨 2+ failed transactions in a row");
        }

        /*

        List<Transaction> distinctTo = lastDay.stream()
            .map(Transaction::getToAccount).distinct().collect(java.util.stream.Collectors.toList());

         */

        List<String> distinctTo = lastDay.stream()
                .map(Transaction::getToAccount) // This produces a Stream of Strings
                .distinct()
